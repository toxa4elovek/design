...
namespace slicedup_behaviors\models\behaviors;
...
class ModelBehavior extends \lithium\core\StaticObject{
	 ...
	 * @var array
	 */
	protected static $_appliedMap = array();
	 ...
	 */
	public static function &attach($model, array $config = array()) {
	...
	protected static function _getAlias($class, $alias) {
		...
		} else if(isset(static::$_aliases[$class]['\\' . $alias])){
			$alias = static::$_aliases[$class]['\\' . $alias];
		}
		return $alias;
	}
	 ...
	 * @param array $apply
	 */
	public function apply(array $apply = array()) {
		$behavior = get_called_class();
		if (!isset(static::$_appliedMap[$behavior])) {
			static::$_appliedMap[$behavior] = array();
		}
