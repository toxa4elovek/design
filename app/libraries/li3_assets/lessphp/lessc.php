...
class lessc
{
    ...
    private static $operatorString; // regex string to match any of the operators

    private static $dtypes = ['expression', 'variable', 'function', 'negative']; // types with delayed computation
        ...
        if ($this->keyword($key) && $this->assign() && $this->propertyValue($value) && $this->end()) {
            ...
            if ($key{0} == $this->imPrefix && strlen($key) > 1) {
                ...
                if ($value[0] == 'list' && $value[1] == ' ') {
                    $value[2][] = ['keyword', '!important'];
                } else {
                    $value = ['list', ' ', [$value, ['keyword', '!important']]];
                }
            }
            $this->append($key, $value);

            if (count($this->env) == 1) {
                return $this->compileProperty($key, [$value])."\n";
            } else {
                return true;
            }
        } else {
    ...
    public function multiplyTags($tags = [' '], $d = null)
    {
        if ($d === null) {
            $d = count($this->env) - 1;
        }
    ...
    public function value(&$value)
    {
        ...
        if ($this->literal('-', false) && $this->variable($vname)) {
            $value = ['negative', ['variable', $this->vPrefix.$vname]];
            return true;
        } else {
            $this->seek($s);
        }
        ...
        if ($this->variable($vname)) {
            $value = ['variable', $this->vPrefix.$vname];
            return true;
        }
        ...
        if ($this->match('(-?[0-9]*(\.)?[0-9]+)('.implode('|', $allowed).')?', $m, !$simpleCase)) {
            ...
            if ($simpleCase) {
                ...
                if ($this->literal('/', false) && $this->unit($right, self::$units)) {
                    $unit = ['keyword', $this->compileValue($unit).'/'.$this->compileValue($right)];
                } else {
    ...
    public function compileBlock($rtags, $env)
    {
            ...
            if (preg_match_all('/{(@[\w-_][0-9\w-_]*)}/', $value[1], $m)) {
                foreach ($m[1] as $name) {
                    if (!isset($replace[$name])) {
                        $replace[$name] = $this->compileValue(['variable', $name]);
                    }
                }
            }
    ...
    public function reduce($var, $defaultValue = ['number', 0])
    {
    ...
    public function evaluate($op, $left, $right)
    {
        ...
        if ($left[0] == 'keyword' || $right[0] == 'keyword' ||
            ...
            return ['keyword', $this->compileValue($left) .' '. $this->compileValue($right)];
        }
    ...
    public function get($name, $env = null)
    {
        if (empty($env)) {
            $env = $this->env;
        }
    ...
    public function getVal($name, $skip = 0, $default = ['keyword', ''])
    {
        $val = $this->get($name);
        if ($val == null) {
            return $default;
    ...
    public function parse($str = null)
    {
        ...
        $this->set('__tags', ['']); // equivalent to 1 in tag multiplication
    ...
    public function __construct($fname = null)
    {
        if (!self::$operatorString) {
            self::$operatorString =
                '('.implode('|', array_map([$this, 'preg_quote'], array_keys(self::$precedence))).')';
        }
