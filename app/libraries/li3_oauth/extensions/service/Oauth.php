...
class Oauth extends \lithium\net\http\Service
{
    ...
    {
        $defaults = ['headers' => true, 'realm' => basename(LITHIUM_APP_PATH)];
     ...
     * @return void
     */
    public function url($url = null, array $options = [])
    {
        $defaults = ['token' => ['oauth_token' => false], 'usePort' => false];
        $options += $defaults;
    ...
    public function sign($options = [])
    {
        $defaults = [
            ...
            'params' => [], 'data' => [],
            'token' => ['oauth_token' => null, 'oauth_token_secret' => null],
        ];
        $options += $defaults;
