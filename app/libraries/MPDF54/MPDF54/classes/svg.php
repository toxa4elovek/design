    ...
    public function svgGradient($gradient_info, $attribs, $element)
    {
        ...
        if ($gradient_info['type'] == 'linear') {
            ...
            for ($i=0; $i<($stops); $i++) {
                ...
                $this->mpdf_ref->gradients[$n]['stops'][] = [
            ...
            $this->mpdf_ref->gradients[$n]['coords']=[$grx1, $gry1, $grx2, $gry2];
            ...
            $this->mpdf_ref->gradients[$n]['extend']=['true','true'];
            if ($trans) {
            ...
            $this->mpdf_ref->gradients[$n]['coords']=[$grx1, $gry1, $grx2, $gry2, abs($grr), abs($grir)  ];
...
*/
        ...
        if (isset($critere_style['fill-opacity'])) {
            ...
            $gs = $this->mpdf_ref->AddExtGState(['ca'=>$opacity, 'BM'=>'/Normal']);
        ...
        if (isset($critere_style['font-family'])) {
            ...
            foreach ($aux_fontlist as $f) {
                ...
                if ((!$this->mpdf_ref->usingCoreFont && in_array($v, $this->mpdf_ref->available_unifonts)) ||
                    ($this->mpdf_ref->usingCoreFont && in_array($v, ['courier', 'times', 'helvetica', 'arial'])) ||
    ...
    public function ImageSVG($data)
    {
        ...
        if (preg_match('/xlink:href=/si', $data)) {
            ...
            for ($i=0; $i<count($links[0]); $i++) {
                ...
                if ($insert) {
                    ...
                    if (preg_match('/y=["\']([^>]*?)["\']/', $inners, $m)) {
                        $y = $m[1];
                    } else {
                        $y = 0;
                    }
                    if (preg_match('/x=["\']([^>]*?)["\']/', $inners, $m)) {
                        $x = $m[1];
                    } else {
                        $x = 0;
                    }
        ...
        if (!function_exists("xml_svg2pdf_start")) {
            function xml_svg2pdf_start($parser, $name, $attribs)
            {
                ...
                case 'line':
                    $arguments = [$attribs['x1'],$attribs['y1'],$attribs['x2'],$attribs['y2']];
