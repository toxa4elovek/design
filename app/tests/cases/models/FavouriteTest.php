...
class FavouriteTest extends AppUnit {
    ...
    {
        $this->rollUp(array('Favourite', 'Pitch', 'User', 'Event'));
    }

    public function tearDown()
    {
        $this->rollDown(array('Favourite', 'Pitch', 'User', 'Event'));
    }

    public function testAdd() {
        $this->assertTrue(Favourite::add(2, 2));
        $record = Favourite::first(array('conditions' => array('user_id' => 2, 'pitch_id' => 2)));
    ...
    public function testAddUser() {
        ...
        $record = Favourite::first(array('conditions' => array('user_id' => 1, 'pitch_id' => 0, 'fav_user_id' => 2)));
        $this->assertTrue(gettype($record), 'object');
        $this->assertTrue(get_class($record), 'lithium\data\entity\Record');
        $event = Event::first(array('conditions' => array(
            'type' => 'FavUserAdded',
            'user_id' => 1,
            'fav_user_id' => 2)));
