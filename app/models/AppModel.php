	...
	public static function __init() {
		...
		self::applyFilter('find', function($self, $params, $chain){
			...
			if(($params['type'] == 'first') && (isset($params['options']['recursiveWith']))) {
				foreach($params['options']['recursiveWith'] as $key => $bindedModel) { 		    		if(is_numeric($key)) {
						$bindedParams = array();
					}else {
					...
					if((in_array($bindedModel, $hasManyModels)) && (!is_null($result))) {
						$recursiveModelFetch = function($result) use ($getKeyName, $bindedModel, $bindedParams, $model) {
			    			...
			    			$result->$alias
			    			= $bindingName::find('all', (array('conditions' => array($keyName => $result->id), 'recursiveWith' => array($bindedModel => $bindedParams)) + $bindedParams));
						};
				...
				foreach($params['options']['recursiveWith'] as $key => $bindedModel) {
					...
					if((in_array($bindedModel, $hasManyModels)) && (!is_null($result))) {
		    			foreach($result as $record) {
			    			...
			    			= $bindingName::find('all', (array('conditions' => array($keyName => $record->id), 'recursiveWith' => array($bindedModel => $bindedParams)) + $bindedParams));
		    			}
		    		}
				}
			}
			...
			if(($params['type'] == 'first') && (isset($params['options']['bindmodel']))) {
				foreach($params['options']['bindmodel'] as $key => $bindedModel) {    
		    		...
		    		if((in_array($bindedModel, $hasOneModels)) && (!is_null($result))) {
		    			...
		    			$result->$alias 
		    			= $bindingName::find('first', (array('conditions' => array($keyName => $result->id)) + $bindedParams));
		    		}
		    		if((in_array($bindedModel, $hasManyModels)) && (!is_null($result))) {
		    			$bindingName = 'app\models\\' . $bindedModel;
		    			$keyName = $getKeyName($model, $bindedModel);
		    			...
		    			$result->$alias
		    			= $bindingName::find('all', (array('conditions' => array($keyName => $result->id)) + $bindedParams));
		    		}    
		    		if((in_array($bindedModel, $belongsToModels) || isset($belongsToModels[$bindedModel])) && (!is_null($result))) {
			    		...
			    		$result->$alias 
			    			= $bindingName::find('first', (array('conditions' => array('id' => $result->{$keyName})) + $bindedParams));
		    		}
		    	}
		    } 
		    	...
		    	foreach($params['options']['bindmodel'] as $key => $bindedModel) {
		    		...
		    		if(((in_array($bindedModel, $hasOneModels))) && (!is_null($result))) {
		    			foreach($result as $record) {
			    			...
			    			$record->{strtolower($bindedModel)} 
			    			= $bindingName::find('first', (array('conditions' => array($keyName => $record->id)) + $bindedParams));
		    			}
		    		}
		    		if((in_array($bindedModel, $hasManyModels)) && (!is_null($result))) {
		    			foreach($result as $record) {
			    			...
			    			$record->{strtolower($bindedModel)} 
			    			= $bindingName::find('all', (array('conditions' => array($keyName => $record->id)) + $bindedParams));
		    			}
		    		}
		    		if((in_array($bindedModel, $belongsToModels) || isset($belongsToModels[$bindedModel])) && (!is_null($result))) {
		    			foreach($result as $record) {
			    			...
			    			$record->{strtolower($bindedModel)} 
			    			= $bindingName::find('first', (array('conditions' => array('id' => $record->{$keyName})) + $bindedParams));
		    			}
		    		}
		    	}
		    }
		...
		self::applyFilter('delete', function($self, $params, $chain){
		    ...
		    foreach($model->hasOne as $key =>$dependant) {
		    	...
		    	$dependantName::remove(array($keyName => $params['entity']->id));
		    }
