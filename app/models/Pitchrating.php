...
class Pitchrating extends AppModel
{
    ...
    public static function setRating($userId, $projectId, $rating)
    {
        ...
        if (!empty($user) && !empty($pitch)) {
            if (!$pitchRating = self::first(['conditions' => ['user_id' => $userId, 'pitch_id' => $projectId]])) {
                $pitchRating = self::create();
            }
            if ($pitchRating->rating != $rating) {
                $rating = ($rating > 5) ? 5 : $rating;
                $rating = ($rating < 1) ? 1 : $rating;
                $pitchRating->set(['rating' => $rating, 'user_id' => $userId, 'pitch_id' => $projectId]);
    ...
    public static function takePart($userId, $projectId)
    {
        ...
        if (!empty($user) && !empty($pitch)) {
            ...
            $pitchRating->set(['trigger' => 1, 'user_id' => $userId, 'pitch_id' => $projectId]);
            return (bool) $pitchRating->save();
        } else {
            return false;
        }
    ...
    {
        $rating = self::first(['conditions' => ['user_id' => $userId, 'pitch_id' => $projectId]]);
