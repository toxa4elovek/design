$(function () {
  /*
   ReactDOM.render(
   <TopPanelContainer data={clientPanelPayload} />,
  ...
  $('#UserShortCompanyName').keyup(function () {
    const charsEntered = $(this).val().length
    const counterElement = $('.character-count')
    const maximumChars = counterElement.data('maxchars')
    let diff = maximumChars - charsEntered
  ...
  if (!$.browser.mobile) {
    ...
    feedbackLink.live('mouseover', function () {
      $(this).css('left', '0')
    })

    feedbackLink.live('mouseout', function () {
      $(this).css('left', '-5px')
    })
  } else {
    feedbackLink.hide()
  ...
  $('#reqsend').on('click', function () {
    if (reqTarget.val() == '0') {
      ...
      $.post('/users/requesthelp.json', sendobj)
    }
    return false
  })
    ...
    FB.login(function (response) {
      if (response.authResponse) {
        ...
        FB.api('/me', function (response) {
          const inviteElement = $('#invite')
          let registerUrl = '/register.json'
          response.accessToken = accessToken
          if ((inviteElement.length == 1) && (inviteElement.val() != '')) {
            registerUrl += '?invite=' + $('#invite').val()
          }
          $.post(registerUrl, response, function (response) {
            let ourResponse = response
  ...
  const user_set_status = function ($form) {
  ...
  const user_popup_register = function () {
  ...
  $(document).on('click', '#fav', function () {
    const link = $(this)
    const data = {'pitch_id': link.data('pitchid')}
    const type = link.data('type')
    let newtype = 'add'
    let newText = 'Следить за проектом'
    if (type == 'add') {
      newtype = 'remove'
      newText = 'Перестать следить'
    }
  ...
  $(document).on('click', '#project-delete-no', function () {
    $('.project-delete-close').click()
    return false
  })

  $(document).on('click', '#project-delete-yes', function () {
    var id = $(this).data('id')
...
function prepareCommentData (comment, result) {
  ...
  if ((comment.user.is_company == 1) && (comment.user.short_company_name != null) && (comment.user.short_company_name != '') && (comment.user.isAdmin == 0)) {
