class ProjectSearchResultsTableRow extends React.Component {
  render () {
    ...
    if (((row.status === '2') && (row.awarded !== '0')) || ((row.status === '2') && (row.type === 'plan-payment'))) {
      ...
      status = <div className='table-link-container'>
                 {linkToClosing}
                 <br/>
                 {downloadDocs}
                 <br/>
    ...
    if (row.type === 'company_project') {
      title = <a href={viewLink} target='_blank'>
                {title}
              </a>
    ...
    return (
    <tr className={row.tableClass} data-id={row.id}>
      <td className='td-title'>
        <span className='newpitchfont'>{title}</span>
        {dateAfterTitle}
      </td>
      <td className='pitches-time' style={statusStyle}>
        {status}
      </td>
      <td className='price' style={{'textAlign': 'left', 'paddingLeft': '48px'}}>
        <span style={{'opacity': '0'}}>-</span><span style={balanceBefore}>{formattedMoney}</span>
        <br/>
        {row.formattedMoney}
      </td>
    </tr>
