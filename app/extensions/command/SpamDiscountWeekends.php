...
class SpamDiscountWeekends extends \app\extensions\command\CronJob
{
    ...
    public function run()
    {
        ...
        Rcache::init();
        $date = date('Y-m-d');
        $pitches = Pitch::all(['conditions' => ['billed' => 0, 'published' => 0, 'category_id' => 1, 'started' => [
        ...
        foreach ($pitches as $pitch) {
            ...
            $cache[$complete_hash] = ['user_id' => $pitch->user->id, 'pitch_id' => $pitch->id, 'email' => $pitch->user->email];
