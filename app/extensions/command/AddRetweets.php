...
class AddRetweets extends \app\extensions\command\CronJob
{
    ...
    public function run()
    {
        ...
        $params = ['count' => 100, 'screen_name' => 'Go_Deer', 'include_entities' => true];
        ...
        $params = ['rpp' => 100, 'q' => $url, 'include_entities' => true];
        $codeTag = $twitterAPI->apiObject->user_request([
            'method' => 'GET',
        ...
        if ($code == 200 && $codeTag == 200) {
            ...
            foreach ($data as $tweet) {
                if (isset($tweet['retweeted_status']) || self::in_array_r('работадлядизайнеров', $tweet['entities']['hashtags']) || (strpos($tweet['text'], ' победил в проекте ') !== false || strpos($tweet['text'], ' заработал ') !== false)) {
                    ...
                    if (!$tweetEvent = Event::first(['conditions' => ['tweet_id' => $tweet['id_str']]])) {
                    ...
                    if (!isset($tweetsDump[$tweet['id_str']])) {
                        ...
                        $params = ['rpp' => 1, 'id' => $tweet['id_str'], 'maxwidth' => '550', 'include_entities' => false];
                        $code = $twitterAPI->apiObject->user_request([
                            'method' => 'GET',
