...
class SmsUslugi
{
    ...
    public function sendedSmsList($start = "0000-00-00", $stop = "0000-00-00", $number = false)
    {
        $result = $this->request("reportNumber", [
            "start" => $start,
            "stop" => $stop,
            "number" => $number
        ]);
    ...
    public function send($params = [], $phones = [])
    {
        ...
        if (isset($params["text"])) {
            $params["text"] = htmlspecialchars($params["text"], null, HTTPS_CHARSET);
        }
    ...
    public function request($action, $params = [], $someXML = "")
    {
    ...
    public function detectCharset($string, $pattern_size = 50)
    {
    ...
    {
        if (!defined("HTTPS_LOGIN")) {
            $this->error("Не удалось подключить конфигурационный файл. Проверьте путь до файла config.php и права на него");
        }
    }
}
...
if (!function_exists("json_decode")) {
    ...
    function json_decode($json, $assoc=false, $limit=512, $options=0, /*emu_args*/$n=0, $state=0, $waitfor=0)
    {
        ...
        #-- result var
        $val = null;
        $FAILURE = [/*$val:=*/ null, /*$n:=*/ 1<<31];
        static $lang_eq = ["true" => true, "false" => false, "null" => null];
        static $str_eq = ["n"=>"\012", "r"=>"\015", "\\"=>"\\", '"'=>'"', "f"=>"\f", "b"=>"\010", "t"=>"\t", "/"=>"/"];
        if ($limit<0) {
