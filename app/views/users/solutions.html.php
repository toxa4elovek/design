<div class="wrapper">
    ...
    <div class="middle">
        <div class="middle_inner conteiners" style="padding:66px 34px 20px 63px !important;margin-top:0;">
            ...
            <?=$this->view()->render(['element' => 'complete-process/filtersmenu'], ['link' => 1])?>
...
font-family: OfficinaSansC Bold, serif; text-align: center; font-size: 25px; font-weight: normal; color: #666666;">Укажите 5 тегов, которые описывают ваше решение</h3>
                    <p style="line-height: 20px; text-shadow: 0 1px 1px #FFFFFF; margin-top: 20px;font-family: OfficinaSansC Book, serif; font-size: 16px; text-align: center; color: #666; margin-bottom: 40px;">
                        Это поможет найти вашу идею тем, кто захочет его купить.<br> Т. о. мы дарим вам возможность продать работу, если та не станет<br> победителем с первого раза. Отметьте работы, которые будут<br> доступны в профиле и на распродаже. <a href="https://godesigner.ru/answers/view/100" target="_blank">Подробнее</a>
                    </p>
                    <?php endif?>
                        ...
                        <li>
                            <div class="selecting_numb">
                                ...
                                <?php if (($this->user->isSolutionAuthor($solution->user_id)) && (($solution->pitch->private != 1) && ($solution->pitch->category_id != 7))):?>
                                    ...
                                    <?=$this->html->link($solution->pitch->title, ['controller' => 'pitches', 'action' => 'view', 'id' => $solution->pitch->id], ['escape' => false, 'style' => 'width: 145px;display: inline-block'])?>
                                </div>
                            </div>
        ...
        <div id="popup-warning" class="popup-warn generic-window" style="display:none;height:300px;">
            <p style="margin-top:120px;">Изменения, внесенные вами в список отображаемых работ в вашем профиле, сохранены!<br> Вы можете просмотреть ваш профиль по этой ссылке:<br> <a href="https://godesigner.ru/user/view/<?=$this->user->getId()?>" target="_blank">https://godesigner.ru/user/view/<?=$this->user->getId()?></a></p>
...
<?=$this->html->script([    '/js/users/office/PushNotificationsStatus.js', 'jcarousellite_1.0.1.js', 'jquery.timers.js', 'jquery.simplemodal-1.4.2.js', 'tableloader.js', 'jquery.timeago.js', 'fileuploader', 'jquery.tooltip.js', 'users/office.js', 'users/preview.js'], ['inline' => false])?>
