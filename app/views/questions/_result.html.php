<section class="howitworks quiz result">
    ...
    <?php if (($firstTime) and ($this->user->isLoggedIn())): ?>
        <?php if ($bestResult): ?>
            ...
            ?>
            <p style="width: 600px;">Oh my God! Вам самое место на платформе GoDesigner!<?php if(!$old):?><br>Ваш аккаунт будет активирован через <?=$remain?> (срок сокращен на 5 дн.)<?php endif?></p>
        <?php else: ?>
            <?php
                $old = false;
                $datetime1 = new DateTime();
                ...
                $remain = $interval->format('%d дн. %h ч. %i мин.');
            ?>
            <p style="width: 600px;">Этого,  однако, недостаточно для участия на платформе GoDesigner, <br> поэтому мы просим вас подтянуть профессиональные навыки! <?php if(!$old):?><br>Ваш аккаунт будет активирован через <?=$remain?><?php endif?></p>
        <?php endif; ?>
    <?php else: ?>
        <?php if ($bestResult): ?>
            <p style="width: 600px;">Oh my God! Вам самое место на платформе GoDesigner!</p>
        <?php else: ?>
            <p style="width: 600px;">Этого,  однако, недостаточно для участия на платформе GoDesigner, <br> поэтому мы просим вас подтянуть профессиональные навыки!</p>
        <?php endif; ?>
    <?php endif; ?>
</section>
