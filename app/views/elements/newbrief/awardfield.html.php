<p>
    ...
    <input type="text" name="" id="award" data-low="<?= $defaultLow ?>" data-normal="<?= $category->normalAward ?>" data-high="<?= $category->goodAward ?>" data-low-def="<?= $defaultLow ?>" data-normal-def="<?= $category->normalAward ?>" data-high-def="<?= $category->goodAward ?>" data-option-title="<?php echo ($category->id == 7) ? 'Награда копирайтеру' : 'Награда Дизайнеру' ?>" data-minimal-award="<?= $low ?>" class="<?php if(!$pitch):?>nitial-price placeholder<?php endif; ?> " placeholder="<?= (date('N') > 5) ? $category->discountPrice : $category->minAward ?>" value="<?php
    if($pitch):
