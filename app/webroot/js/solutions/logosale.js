...
;(function () {
    $(function () {
            ...
            if (gallery.height() - 200 - $(window).scrollTop() < 400 && !isBusy) {
                ...
                $.post(url + page + '.json', paramsSearch, function (response) {
                    ...
                    keys = keys.sort(function (obj1, obj2) {
                        ...
                        if (obj2.rating > obj1.rating) {
                            return 1;
                        }
                        if (obj2.rating < obj1.rating) {
                            return -1;
                        }
                        if (obj2.likes > obj1.likes) {
                            return 1;
                        }
                        if (obj2.likes < obj1.likes) {
                            return -1;
                        }
                        if (obj2.views > obj1.views) {
                            return 1;
                        }
                        if (obj2.views < obj1.views) {
                            return -1;
        ...
        */
        ...
        var recalculateBox = function recalculateBox() {
            var baseWidth = 524,
                filterbox = $('#filterbox');
                ...
                if (currentUserId == result.pitch.user_id) {
                    ...
                    $(".solution-overlay #star-widget").raty({
                        ...
                        click: function click(score, evt) {
                            $.post('/solutions/rating/' + $('input[name=solution_id]').val() + '.json', { "id": result.solution.id, "rating": score }, function (response) {
                                $('.rating-image', '.solution-rating').removeClass('star0 star1 star2 star3 star4 star5');
