...
class TinyImageManager
{
     ...
     */
    public function TinyImageManager()
    {
    ...
    public function Tree($beginFolder)
    {
        ...
        if ($handle) {
            $struct[$beginFolder]['path'] = str_replace([$this->dir['files'], $this->dir['images']], '', $beginFolder);
     ...
     * @return html
     */
    public function DirPath($type, $path='')
    {
        if (!empty($path)) {
    ...
    public function CallDir($dir, $type)
    {
            ...
            while (false !== ($file = readdir($handle))) {
                if ($file != "." && $file != "..") {
                    ...
                    if (is_file($dir.'/'.$file)) {
                        ...
                        $link = str_replace(['/\\', '//', '\\\\', '\\'], '/', '/'.str_replace(realpath(DIR_ROOT), '', realpath($dir.'/'.$file)));
                        $path = pathinfo($link);
                        $path = $path['dirname'];
    ...
    public function UploadFile($dir, $type)
    {
        $dir = $this->AccessDir($dir, $type);
        if (!$dir) {
            return false;
    ...
    public function RenameFile($type, $dir, $filename, $newname)
    {
        $dir = $this->AccessDir($dir, $type);
        if (!$dir) {
            return false;
    ...
    public function bytes_to_str($bytes)
    {
        $d = '';
        if ($bytes >= 1048576) {
            $num = $bytes/1048576;
...
/* 			if($v['general']['width'] > WIDTH_TO_LINK || $v['general']['height'] > HEIGHT_TO_LINK) {
   ...
   <table class="imageBlock0" cellpadding="0" cellspacing="0" filename="'.$v['general']['filename'].'" fname="'.$v['general']['name'].'" type="'.$type.'" ext="'.strtoupper($v['general']['ext']).'" path="'.$v['general']['path'].'" linkto="'.$v['general']['link'].'" fsize="'.$v['general']['size'].'" fsizetext="'.$this->bytes_to_str($v['general']['size']).'" date="'.date('d.m.Y H:i', $v['general']['date']).'" fwidth="'.$v['general']['width'].'" fheight="'.$v['general']['height'].'" md5="'.$v['general']['md5'].'" '.$middle_thumb_attr.'><tr><td valign="bottom" align="center">
    ...
    public function GetThumb($dir, $md5, $filename, $mode, $width=100, $height=100)
    {
