...
class phpExifReader
{
    ...
    */
    ...
    public $MotorolaOrder = 0;
    public $ExifImageWidth = 0; //
    public $FocalplaneXRes = 0; //
    public $FocalplaneUnits = 0; //
    public $sections = [];
    public $currSection = 0;  /** Stores total number fo Sections */

    public $BytesPerFormat = [0,1,1,2,4,8,1,1,2,4,8,4,8];
    ...
    public $ImageReadMode = 3; /** related to $RealMode arrays values */
    public $file =  "";     /** JPEG file to parse for EXIF data */
    public $newFile = 1;   /** flag to check if the current file has been parsed or not. */

    public $thumbnail = ""; /* Name of thumbnail */
    public $thumbnailURL = ""; /* */

    public $exifSection = -1;   // market the exif section index oout of all sections
    ...
    public $caching = false; /* Should cacheing of image thumnails be allowed? */
    public $cacheDir = ""; /* Checkout constructor for default path. */
     ...
     *
     */
    public function phpExifReader($file = "")
    {
     ...
     *
     */
    public function debug($str, $TYPE = 0, $file="", $line=0)
    {
        if ($this->debug) {
     ...
     *
     */
    public function process_SOFn($data, $marker)
    {
        $data_precision = 0;
        $num_components = 0;
     ...
     *
     */
    public function process_COM($data, $length)
    {
     ...
     *
    */
    public function ProcessExifDir($DirStart, $OffsetBase, $ExifLength)
    {
        $NumDirEntries = 0;
        $ValuePtr = [];
            ...
            */
            ...
            switch ($Tag) {
                ...
                case TAG_WHITEBALANCE:
                    ...
                    $tmpArr = ["1"=>"Sunny","2"=>"fluorescent","3"=>"incandescent"];
                ...
                case TAG_METERING_MODE:
                    ...
                    $tmpArr = ["2"=>"center weight","3"=>"spot","5"=>"matrix"];
                ...
                case TAG_EXPOSURE_PROGRAM:
                    $tmp = (int) $this->ConvertAnyFormat($ValuePtr, $Format);
                    $tmpArr = ["2"=>"program (auto)","3"=>"aperture priority (semi-auto)","4"=>"shutter priority (semi-auto)"];
                ...
                case TAG_COMPRESSION_LEVEL:
                    $tmp = (int) $this->ConvertAnyFormat($ValuePtr, $Format);
                    $tmpArr = ["1"=>"Basic","2"=>"Normal","4"=>"Fine"];
                ...
                case TAG_COMPONENT_CONFIG: // need more tests for this
                        ...
                        $tmpArr = ["0"=>"Does Not Exists","1"=>"Y","2"=>"Cb","3"=>"Cr","4"=>"R","5"=>"G","6"=>"B"];
                ...
                case TAG_SENSING_METHOD:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["1"=>"Not Defined","2"=>"One-chip color area sensor","3"=>"Two-chip color area sensor",
                ...
                case TAG_EXPOSURE_MODE:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ['Auto Exposure','Manual Exposure','Auto Bracket'];
                ...
                case TAG_SCREEN_CAP_TYPE:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["Standard","Landscape","Portrait","Night Scene"];
                        $this->ImageInfo['h']["screenCaptureType"] =
                            (isset($tmpArr["$tmp"]) ? $tmpArr["$tmp"] : "Reserved");
                        break;
                case TAG_GAIN_CONTROL:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["None","Low Gain Up","High Gain Up","Low Gain Down","High Gain Down"];
                        $this->ImageInfo['h']["gainControl"] =
                            (isset($tmpArr["$tmp"]) ? $tmpArr["$tmp"] : "Reserved");
                        break;
                case TAG_CONTRAST:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["Normal","Soft","Hard"];
                        $this->ImageInfo['h']["contrast"] =
                            (isset($tmpArr["$tmp"]) ? $tmpArr["$tmp"] : "Reserved");
                        break;
                case TAG_SATURATION:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["Normal","Low Saturation","High Saturation"];
                ...
                case TAG_DIST_RANGE:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["Unknown","Macro","Close View","Distant View"];
                ...
                case TAG_COMPRESS_SCHEME:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["1"=>"Uncompressed","6"=>"JPEG compression (thumbnails only)"];
                ...
                case TAG_IMAGE_P_CONFIG:
                        $tmp = $this->ConvertAnyFormat($ValuePtr, $Format);
                        $tmpArr = ["1"=>"Chunky Format","2"=>"Planar Format"];
    ...
    */
     ...
     *
     */
    public function process_EXT_EXIF($data, $length)
    {
        //print_r($data);
    }
     ...
     *
     */
    public function process_EXIF($data, $length)
    {
     ...
     * @return int
     */
    public function Get32s($val1, $val2, $val3, $val4)
    {
        $val1 = ord($val1);
        $val2 = ord($val2);
        $val3 = ord($val3);
     ...
     *
     */
    public function get32u($val1, $val2, $val3, $val4)
    {
    ...
    //--------------------------------------------------------------------------
    public function ConvertAnyFormat($ValuePtr, $Format)
    {
        $Value = 0;
     ...
     *
     */
    public function saveThumbnail($ThumbFile)
    {
